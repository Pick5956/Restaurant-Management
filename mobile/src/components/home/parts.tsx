...
export function HomeHeading({ icon, title, trailing, onPress }: { icon: AppIconName; title: string; trailing?: string; onPress?: () => void }) {
  const right = trailing ? (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
      <Text style={{ fontSize: 12.5, fontWeight: '600', color: onPress ? palette.primaryInk : palette.placeholder }}>{trailing}</Text>
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 6 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
        ...
        <Text style={{ fontSize: 15.5, fontWeight: '700', color: palette.textStrong }}>{title}</Text>
      </View>
      {onPress && right ? (
  ...
  return (
    <View style={{ flexDirection: 'row', gap: 6 }}>
      {days.map((day) => (
        ...
        >
          <Text allowFontScaling={false} style={{ fontSize: 10.5, fontWeight: '600', color: day.selected ? 'rgba(255,255,255,0.7)' : palette.muted }}>{names[day.weekday]}</Text>
          <Text allowFontScaling={false} style={{ fontSize: 16, fontWeight: '700', lineHeight: 20, color: day.selected ? '#ffffff' : palette.textStrong, fontVariant: ['tabular-nums'] }}>{day.dayOfMonth}</Text>
          <View style={{ width: 4, height: 4, borderRadius: 2, marginTop: 3, backgroundColor: day.hasSales ? (day.selected ? palette.accentMuted : palette.primary) : 'transparent' }} />
        </Pressable>
      ))}
    </View>
  );
...
export function SalesHero({
  ...
  return (
    <Pressable accessibilityRole={onPress ? 'button' : undefined} disabled={!onPress} onPress={onPress} style={({ pressed }) => ({ opacity: pressed ? 0.92 : 1 })}>
      ...
      >
        <Text style={{ fontSize: 12, fontWeight: '600', color: 'rgba(255,255,255,0.82)', letterSpacing: 0.3 }}>{caption}</Text>
        <Text numberOfLines={1} adjustsFontSizeToFit style={{ fontSize: 42, fontWeight: '700', color: '#ffffff', lineHeight: 48, marginTop: 2, fontVariant: ['tabular-nums'] }}>{amount}</Text>
        {reference ? (
          <View style={{ alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 6, backgroundColor: 'rgba(255,255,255,0.18)', borderRadius: 999, paddingVertical: 3, paddingHorizontal: 10 }}>
            <Text style={{ fontSize: 12, fontWeight: '600', color: '#ffffff' }}>{reference}</Text>
          </View>
        ) : null}
        {path && path.line ? (
          ...
          <View style={{ height: H, marginTop: 10, justifyContent: 'flex-end' }}>
            <Text style={{ fontSize: 12, color: 'rgba(255,255,255,0.75)' }}>—</Text>
          </View>
        )}
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
          <Text style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.85)', fontVariant: ['tabular-nums'] }}>{axisStart}</Text>
          {axisNow ? <Text style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.85)', fontVariant: ['tabular-nums'] }}>{axisNow}</Text> : null}
          <Text style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.85)', fontVariant: ['tabular-nums'] }}>{axisEnd}</Text>
        </View>
      </LinearGradient>
    </Pressable>
  );
...
export function StatTile({ icon, label, value, tone = 'neutral', onPress }: {
  ...
  const inner = (
    <View style={{ padding: 12, paddingBottom: 10 }}>
      <View style={{ width: 28, height: 28, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: wash, marginBottom: 8 }}>
        <AppIcon name={icon} size={16} color={ink} />
      </View>
      <Text style={{ fontSize: 11.5, color: palette.placeholder }}>{label}</Text>
      <Text numberOfLines={1} adjustsFontSizeToFit style={{ fontSize: 18, fontWeight: '700', lineHeight: 22, color: ink, fontVariant: ['tabular-nums'] }}>{value}</Text>
    </View>
  );
  return (
    <Pressable accessibilityRole={onPress ? 'button' : undefined} disabled={!onPress} onPress={onPress} style={({ pressed }) => ({ flex: 1, opacity: pressed ? 0.7 : 1 })}>
      <HomeCard radius={20}>{inner}</HomeCard>
    </Pressable>
  );
}
    ...
    return (
      ...
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
          <AppIcon name={card.icon} size={15} color={colours.ink} />
          <Text style={{ fontSize: 11.5, fontWeight: '600', color: colours.ink }}>{card.title}</Text>
        </View>
        <Text style={{ fontSize: 24, fontWeight: '700', lineHeight: 28, color: colours.ink, marginTop: 4, fontVariant: ['tabular-nums'] }}>{card.headline}</Text>
        <Text numberOfLines={1} style={{ fontSize: 12, color: palette.muted, marginTop: 2 }}>{card.detail}</Text>
      </Pressable>
    );
  });
  if (stacked) return <View style={{ gap: 10 }}>{items}</View>;
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 10, paddingHorizontal: 2, paddingBottom: 4 }} style={{ marginHorizontal: -2 }}>
      {items}
    </ScrollView>
  );
}
...
export function TableMap({ cells, columns, legend, onPress }: {
  ...
  return (
    <HomeCard>
      <View style={{ padding: 14, paddingBottom: 12, gap: 8 }}>
          ...
          <View key={rowIndex} style={{ flexDirection: 'row', gap: 8 }}>
            {row.map((cell, index) => {
              ...
              return (
                ...
                >
                  <View style={{ aspectRatio: 1.15, borderRadius: 14, borderCurve: 'continuous', alignItems: 'center', justifyContent: 'center', gap: 3, backgroundColor: look.bg, borderWidth: 1, borderColor: look.border, shadowColor: '#7C2D12', shadowOpacity: cell.state === 'free' ? 0 : 0.10, shadowRadius: 8, shadowOffset: { width: 0, height: 2 }, elevation: cell.state === 'free' ? 0 : 2 }}>
                    <Text allowFontScaling={false} numberOfLines={1} style={{ fontSize: 13, fontWeight: '700', color: look.ink }}>{cell.label}</Text>
                    <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: look.dot }} />
                  </View>
                </Pressable>
              );
            })}
        ...
        <View style={{ flexDirection: 'row', gap: 14, marginTop: 10 }}>
          {([['busy', cellLook.busy.dot], ['bill', cellLook.bill.dot], ...(hasReserved ? [['reserved', cellLook.reserved.dot] as const] : []), ['free', cellLook.free.dot]] as Array<[HomeTableCell['state'], string]>).map(([state, dot]) => (
            <View key={state} style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
              ...
              <Text style={{ fontSize: 11.5, color: palette.muted }}>{legend[state]}</Text>
            </View>
          ))}
        </View>
      </View>
    ...
    <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => ({ opacity: pressed ? 0.7 : 1 })}>
      <HomeCard>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14 }}>
          <View style={{ width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.accentSoft }}>
            <AppIcon name="calendar-outline" size={21} color={palette.primaryInk} />
          </View>
          <View style={{ flex: 1, minWidth: 0 }}>
            <Text style={{ fontSize: 14, fontWeight: '700', color: palette.textStrong }}>{title}</Text>
            <Text numberOfLines={1} style={{ fontSize: 12, color: palette.placeholder }}>{detail}</Text>
          </View>
